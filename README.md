...
> **解决痛点**：通用大模型不懂业务规范，生成内容"假大空"，历史用例无法复用。

- **AI 需求分析**: 自动解析需求文档（PDF/Word/TXT），智能提取业务需求
- **RAG 增强生成**: 基于 ChromaDB 的知识库检索，生成时自动注入相关技术规范和历史用例
...
### 🖥️ UI 自动化测试（Web）
- **双引擎**: Selenium 和 Playwright
- **AI 智能模式**: 基于 Browser-use 框架，AI 理解页面结构并自动完成测试
- 元素库管理，支持多种定位策略
- 页面对象模式（POM），提高脚本可维护性
- 可视化脚本编辑器，支持步骤录制和回放
...
TestFusion 的 RAG 引擎（`rag_engine.py`）基于 ChromaDB 构建，提供语义级别的测试数据和技术规范的检索增强：

### 1) 检索流程（非阻塞漏斗式）
为了避免同步调用大模型过滤导致的 HTTP 接口卡顿，RAG 模块已被彻底移入**异步后台线程**中执行。其核心流程如下：

```
用户提交需求 (0.17s 瞬间响应)
    │
    ▼ (进入异步工作线程)
1. 向量粗筛：ChromaDB 语义检索 Top-K (历史用例 + 测试规范)
    │
    ▼
2. LLM 细筛：后台调用 AI 模型对粗筛内容进行精细去噪，只保留强相关技术点
    │
    ▼
3. 注入生成：纯净的上下文被拼入用例生成 Prompt
```
