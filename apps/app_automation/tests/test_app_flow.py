...
from apps.app_automation.utils.uiautomator2_base import Uiautomator2Base
...
class TestAppFlow:
    ...
    def u2_device(self, device_id, username):
        """uiautomator2 基础环境"""
        u2_base = Uiautomator2Base(device_id=device_id, username=username)

        if not u2_base.setup_device():
            pytest.fail("uiautomator2 设备连接失败")

        yield u2_base

        u2_base.teardown_device()
    
    @allure.story("执行UI Flow")
    def test_execute_ui_flow(self, test_case_id, package_name, execution_id, u2_device, username):
        """执行 UI Flow 测试"""
        # 获取测试用例
        ...
        if package_name:
            with allure.step(f"启动应用: {package_name}"):
                assert u2_device.open_app(package_name), f"应用启动失败: {package_name}"
        else:
            allure.attach(
                "未配置应用包名，跳过启动应用步骤",
                name="启动应用",
