...
class RAGDocument(models.Model):
    ...
    记录上传至向量库的原始文件。文件会经过切片、向量化后存入 ChromaDB，
    此模型仅用于追踪文件元数据。
    """
    STATUS_CHOICES = [
        ('uploaded', '已上传'),
    ...
    raw_text = models.TextField(blank=True, verbose_name='文本内容')
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default='uploaded', verbose_name='状态'
    )
    chroma_doc_id = models.CharField(
        max_length=100, blank=True, null=True,
        verbose_name='ChromaDB文档ID',
        help_text='ChromaDB 中文档的唯一标识，用于删除',
    )
    summary = models.TextField(blank=True, verbose_name='摘要')
    chunk_count = models.IntegerField(default=0, verbose_name='切片数量')
    uploaded_by = models.ForeignKey(
        User, on_delete=models.CASCADE,
        related_name='rag_documents', verbose_name='上传者',
    )
    created_at = models.DateTimeField(default=timezone.now, verbose_name='创建时间')
    updated_at = models.DateTimeField(auto_now=True, verbose_name='更新时间')
    ...
    class Meta:
        ...
        verbose_name = 'RAG知识库文档'
        verbose_name_plural = 'RAG知识库文档'
        ordering = ['-created_at']
...
class AIModelService:
    ...
    async def generate_test_cases(task: TestCaseGenerationTask) -> str:
        ...
        user_message = (
            ...
            f"{'【参考技术规范/历史用例】(请严格遵守):\\n' + task.rag_context + '\\n\\n' if getattr(task, 'rag_context', None) else ''}"
