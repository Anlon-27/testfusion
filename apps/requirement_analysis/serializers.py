...
from .models import (
    ...
    TestCaseGenerationTask, GenerationConfig, RAGDocument,
)
...
class RAGDocumentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = RAGDocument
        fields = [
            'id', 'title', 'file', 'file_url', 'document_type', 'document_type_display',
            'raw_text', 'status', 'status_display', 'chroma_doc_id', 'summary',
            'chunk_count', 'uploaded_by', 'uploaded_by_name', 'created_at', 'updated_at',
        ]
        read_only_fields = [
            'chroma_doc_id', 'chunk_count',
            'uploaded_by', 'status',
        ]
