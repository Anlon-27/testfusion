...
from .models import (
    ...
    TestCaseGenerationTask, GenerationConfig, AIModelService, RAGDocument,
)
from .serializers import (
    ...
    GenerationConfigSerializer, RAGDocumentSerializer,
)
...
class RAGDocumentViewSet(viewsets.ModelViewSet):
    ...
    def process(self, request, pk=None):
        ...
        if doc.status == 'processing':
            return Response({'error': '该文档正在处理中'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            if doc.raw_text:
                ...
                if os.path.exists(file_path):
                    try:
                        if doc.document_type in ('txt', 'md'):
            ...
            if not content:
                doc.status = 'failed'
                doc.save()
                return Response({'error': '无法提取文档内容'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            # 切片入库
            doc_id, chunk_count = engine.add_knowledge(
                content=content,
                source_name=source_name,
                summary=doc.summary or source_name,
            )

            # 更新记录
            doc.chroma_doc_id = doc_id
            doc.chunk_count = chunk_count
            doc.status = 'ready'
            doc.save()

            logger.info(f"RAG文档处理完成: {source_name}, chroma_doc_id={doc_id}")
            return Response({
                'message': '文档处理完成',
    ...
    def search(self, request):
        ...
        try:
            ...
            context, sources = engine.search_context(
                query, top_k_knowledge=top_k, top_k_history=top_k
            )
            ...
            return Response({
                ...
                'sources': sources,
                'context': context[:2000] if context else '无匹配结果',
            })

        except Exception as e:
            logger.error(f"RAG检索测试失败: {e}")
            return Response({'error': f'检索失败: {str(e)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

    def perform_destroy(self, instance):
        """删除文档同时清理 ChromaDB 中的向量"""
        chroma_id = instance.chroma_doc_id
        if chroma_id:
            try:
                from .rag_engine import RAGEngine
                engine = RAGEngine()
                engine.delete_document(chroma_id)
                logger.info(f"已从 ChromaDB 删除文档 {chroma_id}")
            except Exception as e:
                    ...
                    try:
                        ...
                        def execute_task():
                            try:
                                ...
                                # 创建独立的事件循环，供 RAG 和 AI 生成共同使用
                                loop = asyncio.new_event_loop()
                                asyncio.set_event_loop(loop)

                                # -- RAG 增强：检索知识库并注入上下文 (已平移至异步线程，解决同步网络阻塞) --
                                if task.enable_rag:
                                    try:
                                        logger.info(f"任务 {task.task_id} 启动异步后台 RAG 知识库检索与过滤...")
                                        from .rag_engine import RAGEngine
                                        engine = RAGEngine()

                                        # 用需求文本检索知识库和历史用例
                                        raw_context, sources = engine.search_context(
                                        ...
                                        if raw_context:
                                            # 使用 LLM 过滤去噪（使用 writer 模型）
                                            writer_config = task.writer_model_config
                                            if writer_config and raw_context:
                                                filtered_context = loop.run_until_complete(
                                                    engine.filter_rag_context(
                                                        query=task.requirement_text,
                                                        raw_context=raw_context,
                                                        llm_api_key=writer_config.api_key,
                                                        llm_base_url=writer_config.base_url,
                                                        llm_model_name=writer_config.model_name,
                                                    )
                                                )

                                                if filtered_context and '无相关参考资料' not in filtered_context:
                                                    task.rag_context = filtered_context
                                                    task.save(update_fields=['rag_context'])
                                                    logger.info(
                                                        f"RAG 增强: 任务 {task.task_id} 从 {len(sources)} 个来源检索并过滤后得到参考上下文 "
                                                        f"(过滤后 {len(filtered_context)} 字符)"
                                                    )
                                    except Exception as rag_error:
                                        logger.warning(f"任务 {task.task_id} 的 RAG 检索失败（不影响主生成流程）: {rag_error}")
